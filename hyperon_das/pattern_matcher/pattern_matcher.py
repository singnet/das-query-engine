...
class OrderedAssignment(Assignment):
    ...
    def check_negation(self, negation: Assignment) -> bool:
        if isinstance(negation, OrderedAssignment):
            ...
            return (
                check != CompatibilityStatus.EQUAL
                and check != CompatibilityStatus.FIRST_COVERS_SECOND
            )
        else:
...
class UnorderedAssignment(Assignment):
    ...
    def freeze(self):
        ...
        self.hash = hash(
            tuple(
                [
                    hash(frozenset(self.symbols.items())),
                    hash(frozenset(self.values.items())),
                ]
            )
        )
        return True
    ...
    def check_negation(self, negation: Assignment) -> bool:
        if isinstance(negation, OrderedAssignment):
            ...
            return all(
                not self.contains_unordered(unordered_negation)
                for unordered_negation in negation.unordered_mappings
            )
    ...
    def is_covered_by_ordered(self, ordered_assignment) -> bool:
        ...
        return all(count <= 0 for count in copy.symbols.values()) and all(
            count <= 0 for count in copy.values.values()
        )
    ...
    def compatible(self, other) -> bool:
        ...
        return (
            sum_value_count_other >= sum_symbol_count_self
            and sum_value_count_self >= sum_symbol_count_other
        )
...
class CompositeAssignment(Assignment):
    ...
    def check_negation(self, negation: Assignment) -> bool:
        if isinstance(negation, OrderedAssignment):
            ...
            return all(
                not assignment.contains_unordered(negation)
                for assignment in self.unordered_mappings
            )
        else:
            for assignment in self.unordered_assignments:
                if all(
                    assignment.contains_unordered(negation_assignment)
                    for negation_assignment in negation.unordered_assignments
                ):
                    return False
            return True
    ...
    def contains_ordered(self, ordered_assignment) -> bool:
        return all(
            assignment.contains_ordered(ordered_assignment)
            for assignment in self.unordered_mappings
        )

    def contains_unordered(self, unordered_assignment) -> bool:
        return all(
            assignment.contains_unordered(unordered_assignment)
            for assignment in self.unordered_mappings
        )
...
class Link(Atom):
    ...
    def _assign_variables(
        ...
        assert len(link_targets) == len(
            self.targets
        ), f'link_targets = {link_targets} self.targets = {self.targets}'
        answer = None
        if self.ordered:
            answer = OrderedAssignment()
    ...
    def _apply_assignment(self, assignment: OrderedAssignment, db: IAtomDB) -> str:
        targets = []
    ...
    def apply_assignment(self, assignment: OrderedAssignment, db: IAtomDB) -> str:
        targets = []
        for t in self.targets:
            if type(t) is Node:
                targets.append(t.get_handle(db))
    ...
    def matched(
        ...
        if any(handle == WILDCARD for handle in target_handles):
            ...
            if DEBUG_LINK:
                print(f'len(answer.assignments) = {len(answer.assignments)}')
            if DEBUG_LINK:
                print(f'answer.assignments = {answer.assignments}')
            if DEBUG_LINK:
                print('matched()', f'leaving 1 self = {self}')
            return bool(answer.assignments)
        else:
            if DEBUG_LINK:
                print('matched()', f'leaving 2 self = {self}')
    ...
    def matched(
        ...
        for term in self.terms:
            ...
            if not term_answer.assignments:
                if DEBUG_OR:
                    print(f'term_answer empty: {term}')
                continue
            if not or_answer.assignments:
                if DEBUG_OR:
                    print(f'First term: {term}')
            ...
            if DEBUG_OR:
                print(f'or_answer after extending:\n{or_answer}')
        if negative_terms:
            joint_negative_term = And([t.term for t in negative_terms])
            if DEBUG_NOT:
                print(f'Joint negative term: {joint_negative_term}')
            term_answer = PatternMatchingAnswer()
    ...
    def matched(
        ...
        for assignment in and_answer.assignments:
            ...
            if all(assignment.check_negation(tabu) for tabu in forbidden_assignments):
                ...
                if DEBUG_AND:
                    print(f'Excluding {assignment}')
        if DEBUG_AND:
            print(f'AND result = {answer}')
        return bool(answer.assignments)
