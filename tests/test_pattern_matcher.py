...
def build_unordered_assignment(d):
...
class TestPatternMatchingAnswer:
    ...
    def test_patterns(self, database):
        ...
        check_pattern(
            ...
            LinkTemplate(
                'Inheritance',
                [
                    TypedVariable("V1", "Concept"),
                    TypedVariable("V2", "Concept"),
                ],
                True,
            ),
            True,
    ...
    def test_basic_matching(self, database):
        ...
        assert not Link(
            "blah", [Node("Concept", "human"), Node("Concept", "mammal")], True
        ).matched(db, answer)
        assert not Link(
            "Inheritance",
        ...
        assert Link(
            "Similarity",
            [Node("Concept", "snake"), Node("Concept", "earthworm")],
            False,
        ).matched(db, answer)
        assert Link(
            "Similarity",
            [Node("Concept", "earthworm"), Node("Concept", "snake")],
            False,
        ).matched(db, answer)
        assert not Link(
            "Similarity",
            [Node("Concept", "earthworm"), Node("Concept", "vine")],
            False,
        ).matched(db, answer)
        assert not Link(
            "Similarity",
            [Node("Concept", "vine"), Node("Concept", "earthworm")],
            False,
        ).matched(db, answer)
        assert Link(
            "Similarity",
            [Node("Concept", "snake"), Node("Concept", "vine")],
            False,
        ).matched(db, answer)
        assert Link(
            "Similarity",
        ...
        assert not Link(
            "Similarity",
            [Node("Concept", "snake"), Node("blah", "earthworm")],
            False,
        ).matched(db, answer)
        assert not Link(
            "Similarity",
            [Node("blah", "snake"), Node("Concept", "earthworm")],
            False,
        ).matched(db, answer)
        assert not Link(
            "Similarity",
            [Node("blah", "earthworm"), Node("Concept", "snake")],
            False,
        ).matched(db, answer)
    ...
    def test_unordered_assignment_sets(self):
    ...
    def test_evaluate_compatibility(self):
        ...
        a2 = build_ordered_assignment({'v1': '1', 'v2': '2'})
        a3 = build_ordered_assignment({'v1': '1', 'v2': '2', 'v3': '3'})
        a4 = build_ordered_assignment({'v1': '1'})
        a5 = build_ordered_assignment({'v3': '3', 'v2': '2'})
        a6 = build_ordered_assignment({'v3': '3', 'v4': '4'})
        a7 = build_ordered_assignment({'v4': '1', 'v5': '2', 'v6': '3'})
    ...
    def test_join(self):
        a1 = build_ordered_assignment({'v1': '1', 'v2': '2'})
        a2 = build_ordered_assignment({'v1': '1'})
        a3 = build_ordered_assignment({'v1': '1', 'v2': '2'})
        a4 = build_ordered_assignment({'v1': '1', 'v2': '2', 'v3': '3'})
        a5 = build_ordered_assignment({'v2': '2', 'v3': '3'})
        a6 = build_ordered_assignment({'v3': '3', 'v4': '4'})
    ...
    def test_check_negation(self):
        a1 = build_ordered_assignment({'v1': '1', 'v2': '2'})
        a2 = build_ordered_assignment({'v1': '1'})
        a3 = build_ordered_assignment({'v1': '2', 'v2': '1'})
        a4 = build_ordered_assignment({'v1': '1', 'v2': '2', 'v3': '3'})
        a5 = build_ordered_assignment({'v1': '1', 'v2': '4'})
        a6 = build_ordered_assignment({'v4': '1', 'v2': '2'})
        a7 = build_ordered_assignment({'v4': '1', 'v5': '2', 'v6': '3'})
        a8 = build_ordered_assignment({'v1': '4', 'v2': '5', 'v3': '6'})
        a9 = build_ordered_assignment({'v1': '1', 'v2': '1'})
        a10 = build_ordered_assignment({'v1': '1', 'v2': '1', 'v4': '2'})
        a11 = build_ordered_assignment({'v1': '3', 'v2': '2', 'v3': '1'})

        b1 = build_unordered_assignment({'v1': '1', 'v2': '2', 'v3': '3'})
        b2 = build_unordered_assignment({'v1': '1', 'v2': '2'})
